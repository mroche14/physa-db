...
release-check: release-plz-schema release-plz-dry-run

# Validate release-plz.toml against the schema generated by the pinned release-plz binary.
release-plz-schema:
    mise exec -- python3 .github/scripts/check-release-plz-config.py

# Exercise release-plz update in a temporary worktree, leaving this branch untouched.
release-plz-dry-run:
    mise exec -- .github/scripts/release-plz-dry-run.sh

# Check commit subjects on the current branch against Conventional Commits.
check-commits base="origin/main":
    git log --format=%s {{base}}..HEAD | .github/scripts/check-conventional-commits.sh --subjects -
